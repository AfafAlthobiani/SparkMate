...
import { Info, Sparkles, Share2, Twitter } from 'lucide-react';

interface ServicesProps {
  onBook: (service: Service) => void;
}

const getShareLinks = (service: Service, origin: string) => {
  const shareUrl = origin ? `${origin}/#services` : 'https://ais-pre-s3nivrzzrpbkvoveasnmtb-87752171815.europe-west3.run.app/#services';
  const shareText = `أنصحكم بتجربة خدمة "${service.name}" من رفيق اللمعة للتنظيف الشامل بالمدينة المنورة! ✨🧼\n\n${service.desc}\n\nتصفح الخدمة واحجز من هنا:\n${shareUrl}`;
  return {
            ...
            >
              ...
              <div className="p-6 pt-5 flex-1 flex flex-col">
                ...
                {(() => {
                  ...
                  const links = getShareLinks(service, currentOrigin);
                  return (
                    <div className="mt-5 pt-4 border-t border-dashed border-[#e0f7f7] flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[13px] text-[#4f5e71] font-bold flex items-center gap-1 select-none">
                          <Share2 className="w-3.5 h-3.5 text-[#3476A8]" />
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <a
                            ...
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-[#25d366]/10 text-[#20ba56] hover:bg-[#25d366] hover:text-white text-xs font-black transition-all duration-300 shadow-xs border border-[#25d366]/20"
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-slate-100 text-slate-800 hover:bg-slate-900 hover:text-white text-xs font-black transition-all duration-300 shadow-xs border border-slate-200"
                            onClick={(e) => e.stopPropagation()}
